...
pub async fn aplicar_aumento_precios(ajuste: web::Json<AjustePrecio>, client: web::Data<Client>) -> impl Responder {
                        ...
                        match client_ref.put(firebase_url).json(&json_data).send().await {
                            Ok(put_response) => {
                                if put_response.status().is_success() {
                                    let notification_message = format!("Todos los precios aumentaron {:.2} %", ajuste.porcentaje);
